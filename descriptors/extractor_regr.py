...
def run_full_pipeline(log_folder, xlsx_path, max_features, target="ln(kobs)",
        ...
        try:
            ...
            if nbo_content:
                ...
                if (Ar_f is None) or (Ar_g is None):
                    ...
                    # Second fallback: robust BD+geometry parsing from (1).py
                    if (Ar_f is None) or (Ar_g is None):
                        try:
                            c1r, c2r, fr, gr, elems_geo, coords_geo = derive_fg_from_geometry_robust(log_text)
                            if (Ar_c is None) and (c1r is not None): Ar_c = c1r
                            if (Ar_e is None) and (c2r is not None): Ar_e = c2r
                            if (Ar_f is None) and (fr  is not None): Ar_f = fr
                            if (Ar_g is None) and (gr  is not None): Ar_g = gr
    ...
    # === Fill missing F/G using robust resolver from (1).py ===
    try:
        # Build a simple resolver from unique_ar_df mapping
        _path_map = dict(zip(unique_ar_df['Ar_key'].astype(str), unique_ar_df['log_path']))
        def _resolver(compound):
            key = str(compound).lstrip('A')
            # Try direct match on Ar_key, else fallback to numeric key
            return _path_map.get(key, os.path.join(log_folder, f"{key}.log"))
        df = fill_missing_ring_FG(df, _resolver, store_mode='index', flip_on_missing=True, verbose=False)
    except Exception as _e:
        print(f"[fill_missing_ring_FG] skipped due to: {_e}")
